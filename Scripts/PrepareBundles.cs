...
#pragma warning restore 649
		...
		public ReturnCode Run()
		{
			...
			foreach (string assetGuid in assetGuids)
			{
				...
				{
					string modname = m_ModificationParameters.TargetFolderName;
					string[] split = assetPath.Split('/');
					string assetmod = split[2];
					string assetname = split[4];

					if (assetmod != modname)
					{
						Debug.Log($"Asset {assetname} ({assetmod}) is from outside the current project, skipping.");
						continue;
					}
				}
				// DP EDIT END
